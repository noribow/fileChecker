...
- パスワード保護（暗号化）された圧縮ファイル・エントリの復号には対応しない（10.7参照）。
...
### 10.7 暗号化・パスワード保護された圧縮ファイルの扱い（2026-08-30 決定、2026-09-01 改訂）

- 本アプリはパスワード保護（暗号化）された圧縮ファイル・エントリの復号には対応しない。パスワードの登録・管理機能も設けない。
- パスワード保護されたアーカイブ（エントリ）を検出した場合は、復号を試みずエラー扱いとしてスキップする（5.1のエラーハンドリング方針に従う）。
- **改訂の経緯**: 当初はユーザーが事前登録したパスワードによる復号（モード2）を選択可能にする方針だったが、2026-09-01の要件見直しで、本アプリ内にパスワード付き圧縮の復号機能自体を持たない方針に変更した。これに伴い、登録パスワードの保存方式・管理画面（旧10.9）およびマスターパスワードによる保護方式（旧10.10）の決定は不要となったため削除した。
- **未決事項として残す**（9章参照）:

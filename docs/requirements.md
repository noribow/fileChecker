...
当初リストにはなかったが、決定事項の検討過程で新たに見つかった論点が6点あった（詳細・追跡は
`docs/open-decisions.md`参照）。うち4点は決定済み、2点は「今回は採用しない／将来の検討課題とする」
という将来の再検討条件付きの設計判断であり、その再検討条件自体が未決事項として追跡中:

- ~~スキャン履歴（`scan_run`/`check_run`等）の自動保持期間・自動プルーニング方針（§10.12で言及）~~
  → §10.22・§10.23で決定
- ~~エラーハンドリングのテキストログファイルのローテーション・保持期間（§10.17で言及）~~
  → §10.22で決定
- ~~非Solid版・ZSTD版7z（`NLZMA`/`SZSTD`/`NZSTD`）への対応要否（§10.19で言及）~~ → §10.24で決定
- ~~再構成先フォルダに参照ファイルと同名だが内容が異なるファイルが既存する場合の上書き/退避方針
  （§10.20で言及）~~ → §10.24で決定
- 重複チェックにおける部分ハッシュ（先頭N KB/MB等）導入の再検討（§10.2で言及）— TB級データでの
  I/O負荷が実運用上問題になった場合の再検討条件・判断基準は未定。`docs/open-decisions.md` 7.5で追跡中。
- 走査と並行して逐次比較を行う「中途比較」（ストリーミング／インクリメンタル方式、§10.3で言及）の
  将来採用可否 — 検討時期・トリガー条件は未定。`docs/open-decisions.md` 7.6で追跡中。
...
- **今回のスキーマ設計では対応しない点**: リムーバブルメディア識別のフォールバック方針は§10.21で
  ...
  期間・自動プルーニング方針は§10.22・§10.23で決定済み（初回リリースでは自動保持期間・自動プルーニング
  機能を設けず無期限保持とする）。いずれも`removable_media`／`scan_run`の追加カラムで将来拡張できる
  設計としてある。
...
flowchart TD
    ...
    Home --> ScanBrowser["スキャン状況（4ペイン）\nフォルダ/メディア一覧 → ファイル一覧 → 圧縮ファイル内一覧"]
```
...
**スキャン状況**（§10.25で追加。整合性／重複チェックを実行しなくても、スキャンしただけの
フォルダ・リムーバブルメディアの中身をそのまま確認できる4ペイン画面）
- 左上: スキャン済みフォルダ一覧（各行にそのフォルダから生成されたお手本セットがあれば表示）。
- 左下: スキャン済みリムーバブルメディア一覧。
- 右上: 左側で選択した`scan_run`のトップレベルファイル一覧（アーカイブ自身の行は含むが中身は含まない）。
- 右下: 右上で圧縮ファイル（📦）を選んだ場合の、その内部エントリ一覧。
- 右上・右下は§10.25の通りページング取得・仮想スクロール表示とし、1回のスキャンが数十万〜百万件
  （§4）に達しても一覧全体を一括転送しない。
...
- **テキストログファイルへの記録（副）**: DBの簡潔な記録に加え、アプリの設定フォルダ内（§10.9の登録
  ...
  ログファイル自体のローテーション・保持期間は§10.22で決定済み（ローテーション・自動削除は行わず、
  永続的に保持する）。
...
- **非Solid版・ZSTD版（`NLZMA`/`SZSTD`/`NZSTD`）の扱い**: 今回はSolid-LZMA版を採用形式として決定する
  ...
  こととしたが、その後§10.24で「初回リリースではSolid-LZMA版のみ対応」と決定済み。
...
### 10.24 再構成機能の残課題2点（2026-09-04 決定）

§10.20（6.3）の決定文中に残されていた2つの実装詳細（`docs/open-decisions.md` 7.3・7.4）を決定する。

#### 7.3: 非Solid版・ZSTD版7z（`NLZMA`/`SZSTD`/`NZSTD`）への対応要否

- **決定**: 初回リリースでは**Solid-LZMA版（RV7Z、§10.19で採用済み）のみ**に対応する。他バリアント
  （非Solid版`NLZMA`、ZSTD版`SZSTD`/`NZSTD`）への対応は見送る。
- **理由**: §10.19で「RomVault実装で"today"のデフォルトとされる方式」としてSolid-LZMA版を既に採用
  しており、これが現状の主流形式である。他バリアントへの対応は実装コストを追加するだけで、その需要が
  実際にあるかは現時点で不明。再構成機能はまだ実運用されていない新規機能であり、需要が明確になって
  から拡張する方が過剰実装を避けられる。なお、この決定は**書き出し（生成）**時の対応形式のみに影響し、
  **読み取り**（整合性チェック・重複チェックでの展開・ハッシュ照合）は§10.5の決定通り元々全形式に
  対応済みで、本節の影響を受けない。

#### 7.4: 再構成先に参照ファイルと同名だが内容が異なるファイルが既存する場合の扱い

- **決定**: **無条件で上書きする**。退避・バックアップは行わない。
- **理由**: 再構成機能の目的は、指定した再構成先フォルダの内容をお手本セットに一致させることそのもの
  である。同名だが内容が異なるファイルは、定義上「お手本セットに一致しない、正しくない内容」であり、
  上書きして正しい内容に置き換えることが再構成の目的に直接合致する。退避・バックアップを挟むと実行
  ロジック・完了報告の実装が複雑になる一方、再構成先フォルダは元々ユーザーが「お手本セットに合わせて
  再構成する対象」として明示的に指定した場所であり、そこにある無関係なデータの保全までは再構成機能の
  責務としない、という判断。ユーザーが誤って重要なフォルダを再構成先に指定した場合の保護は、本節の
  対象外とする。

### 10.25 スキャン状況の確認画面（4ペイン）（2026-09-09 決定）

- **背景**: 整合性チェック・重複チェックを実行しなくても、スキャンしただけのフォルダ・リムーバブル
  メディアの中身（トップレベルのファイル一覧、圧縮ファイルであればその内部一覧）を直接確認する手段が
  なかった。既存の「スキャン履歴」画面（§10.14）は`scan_run`単位のサマリ（対象・日時・ステータス・
  ファイル数）のみ、「リムーバブルメディア管理」画面はメディアの識別情報のみを表示しており、いずれも
  `scanned_file`単位の一覧を持たない。
- **決定**: 独立したサイドバー項目「スキャン状況」として、4ペイン構成の画面を追加する（§10.14の画面
  一覧に追記済み）。
  - 左上: スキャン済みフォルダ一覧（`target_type='folder'`の`scan_run`）。各行が生成元となった
    お手本セット（`reference_set.generated_from_scan_run_id`が一致するもの）を持つ場合は名前・
    バージョンを併記する。
  - 左下: スキャン済みリムーバブルメディア一覧（`target_type='removable_media'`の`scan_run`）。
  - 右上: 左側で選択した`scan_run`のトップレベルエントリ一覧（`parent_archive_file_id IS NULL`、
    §10.5。圧縮ファイル自身の行は含むが、その中身は含まない）。
  - 右下: 右上で圧縮ファイル（`archive_format`が設定された行）を選んだ場合の、その直下エントリ一覧
    （`parent_archive_file_id`が選択行の`id`と一致するもの）。二重に入れ子になった圧縮ファイル
    （圧縮ファイルの中の圧縮ファイル）をさらに展開する3段目以降のドリルダウンは、本画面のスコープ
    外とする（既存の再構成機能同様、必要になれば別途検討）。
- **一覧取得方式（ページング・仮想スクロール）**: 1回の`scan_run`のトップレベル一覧は§4の非機能要件
  （数十万〜百万ファイル）により大量件数になり得るため、右上・右下パネルは全件を一括転送せず、
  `offset`/`limit`でページング取得する。フロントエンドはこれを仮想スクロール（スクロールバー自体は
  ネイティブの連続スクロールに見えるが、実際にDOM要素として存在するのは表示中の行とその前後の
  少数のみ）で消費する。1ページ目の応答に`COUNT(*) OVER()`で総件数を含めることで、一覧全体を
  ロードせずにスクロールバーの正しい長さを算出できるようにした。
  - **理由**: 「スクロールバーによる自然な操作感を保ちながら大量件数でも重くならない」ことを両立する
    唯一の方式が仮想スクロールであるため（単純な全件描画はスクロールバーの見た目は自然だが大量件数で
    重くなり、逆にページ送りUIは軽量だが連続スクロールできない）。
  - **検証対象外**: このページング・仮想スクロールの工夫が実際に数十万〜百万行規模で妥当かどうかの
    実測・チューニングは、P14（非機能要件の検証）の範囲であり、本節の決定時点では検証していない。

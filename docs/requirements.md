...
- 登録パスワードの保存方式（キーチェーン連携等、セキュリティ上安全な実装方式）
- パスワード登録UI・一括設定の具体的な実装方法
...
- アプリ設定で、ユーザーが以下いずれかの動作を選択できるようにする:
  ...
  2. **登録済みパスワードで復号を試行する**: ユーザーが事前に登録したパスワードを用いて復号を試みる。パスワードは圧縮ファイル形式ごとの個別設定、および複数形式への一括設定の両方に対応する（登録UI・保存の具体的な実装方法は別途検討）。
- モード2で、登録済みのいずれのパスワードでも復号できなかった場合（該当パスワードなし、パスワード誤り等）も、エラー扱いとする。
- **未決事項として残す**（9章参照）:
  - 登録パスワードの保存方式（SQLite DBへの平文格納は避けるべきであり、OSの資格情報マネージャ／キーチェーン連携や暗号化保存等、セキュリティ上安全な具体的な実装方式）
  - パスワード登録UI・一括設定の具体的な実装方法
